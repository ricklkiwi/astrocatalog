...
| 1     | MVP v1.0                    | Scanning, app shell, targets, sessions, calibration, retrieval/handoff, stats, packaging         | P1-01 … P1-37 (+P1-12a, P1-13a, P1-18a) | 40          |
...
Total: **90 issues**. Phase 1 is the critical path to beta; a closed beta (PRD §9: 50 users) should start after milestone M4 below.
...
| M3 Target library         | Targets auto-grouped, integration times correct, review queue functional            | P1-10…P1-16 (incl. P1-12a, P1-13a)           |
| M4 Sessions & calibration | Sessions auto-detected; calibration matching + gap report                           | P1-17…P1-23 (incl. P1-18a) → **closed beta** |
...
5. **Performance budgets** (PRD §8.4) are enforced by benchmark tests (P0-07, P1-33): 10k headers scanned < 5 min, 100k-file library loads < 3 s, target dashboard < 1 s.
...
P0 (foundations) ──► P1 scanning (P1-01..09) ──► targets (P1-10..16)
                                    ...
                                    └──► thumbnails (P1-26..27)
targets+sessions ──► file browser/handoff (P1-25..29)
all M2-M5 ──► perf/packaging/release (P1-33..36)
...
- **Benchmarks:** synthetic 10k/100k-file libraries in CI with regression thresholds, plus a realistic I/O benchmark pack for cold/warm cache runs, external-drive assumptions, p50/p95 reporting, and degraded-mode behavior.
...
| Large-library performance                           | Benchmarks in CI from Phase 0; realistic I/O benchmark pack; virtualized UI; staged pipeline |
...
- Versioning: semver; v0.x during Phase 1, v1.0.0 at M6. Full processing-project workflow is v1.x after the v1.0 archive/retrieval loop is useful.
