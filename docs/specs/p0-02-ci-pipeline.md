...
- [ ] New domain logic is in packages/core with no Electron/fs imports — N/A; no domain logic added. `pnpm -r lint`, run as part of both `ci.yml` and `CONTRIBUTING.md`'s local gate, continues to enforce the existing `packages/core` `no-restricted-imports` rule on every PR from now on — CI's introduction *is* the mechanism, not a change to it.
