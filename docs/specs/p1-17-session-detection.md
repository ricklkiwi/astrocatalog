...
- [ ] **LOCK-4** — Given any locked group, its emitted `sessionId` is its members' `existingSessionId` verbatim, never `null` and never a different id — **fails under:** `detect-sessions.ts` routing locked groups through the majority/tie-break id-selection helper used for unlocked runs.
