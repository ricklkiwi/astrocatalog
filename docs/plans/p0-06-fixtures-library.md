...
acceptable. Every fixture is therefore *synthesized byte-exactly to each program's documented
header conventions* (public docs cited per manifest entry) and marked honestly in its
...
criterion's "real-world" wording is interpreted as "real-world *conventions*, faithfully
...
covers at minimum every PRD §8.2 *critical* keyword present in the file — this is the
...
deliberately emit *broken* output (unpadded blocks, missing END, overlong/short cards,
...
- **SGPro (5):** light with 7-fractional-digit `DATE-OBS`/`DATE-LOC` timestamps and CREATOR
  keyword; light *without* FOCALLEN (documented intermittent omission); dark; flat; light
...
- **APT (4):** DSLR light (no FILTER keyword — exercises DD-005 `None` canonical filter),
  ...
  dark; flat.
- **SharpCap (4):** EAA-style light with *no OBJECT* (needs-review bucket path, DD-005 step
...
valid samples: a PixInsight-style unit (Image element with `FITSKeyword` elements *and* XISF
...
OffsetTimeOriginal, so P1-03's "UTC normalization using EXIF offset *when present*" has both
...
- CONTINUE card whose continued string never closes its quote — must be a *malformed* fixture
  with `BAD_CONTINUE`, distinct from the *valid* CONTINUE fixture with LONGSTRN.
...
- [x] Timestamps stored UTC: all DATE-OBS fixture values are UTC; DATE-LOC fixtures exist
      precisely to test that local time is *not* what gets persisted
- [x] Long-running work through worker queue: N/A — dev-time CLI tooling only, nothing runs
      inside the app
- [x] Performance budgets: no runtime code paths touched; this issue *enables* P0-07's
...
3. **One manifest per fixture set** (`fits/`, `xisf/`, `raw/`; malformed entries live inside
   ...
   shared JSON Schema.
4. **Fixture binaries are committed** *and* regenerable from checked-in definitions, with a
