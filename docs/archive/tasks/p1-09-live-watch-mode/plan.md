...
- **Watcher-limit fallback while a manual "Scan now" is independently in flight for the same
  folder**, or a debounce firing while an *unrelated* manually-triggered scan is queued for that
  folder: `WatchManager`'s in-flight guard only tracks jobs *it* enqueued (it has no visibility
...
- [x] Non-destructive: the watcher only subscribes to fs change *notifications* (chokidar's
