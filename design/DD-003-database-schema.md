...
watch_folders(id, path, drive_label, is_active, last_scan_at, created_at)
...
targets(
  ...
  notes, thumbnail_frame_id, created_at
)

target_aliases(id, target_id → targets, alias_normalized, source) -- 'builtin'|'user'

-- Normalized filters with canonical grouping
filters(id, raw_name, canonical_name, band_type) -- 'L','R','G','B','Ha','OIII','SII','UVIR','none'…
...
master_frames(
  ...
  created_date, sub_count, notes
)
master_frame_subs(master_frame_id, frame_id)   -- which raw subs built the master

-- Processing workflow
processing_projects(id, target_id, name, version_label, status, software, notes, created_at)
project_inputs(project_id, frame_id | master_frame_id)     -- polymorphic via type column
processed_images(id, project_id, file_id, format, is_final, published_url, created_at)

-- Infrastructure
scan_jobs(id, watch_folder_id, status, files_seen, files_added, files_updated, started_at, finished_at, error)
thumbnails(frame_id, cache_path, generated_at)
